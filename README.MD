...
The FS Management Tool in Windows, beyond not being available for Mac or Linux clients, can only display up to 10k locks or file handles which is a limit that is often exceeded among Qumulo customers while this tool has been field-tested to handle in excess of 120,000 locks.
...
Running this code as a Kubernetes cluster deployed in the same data center as the Qumulo cluster allowed the resolution of 127,000 locks in around 40 seconds. (4 x QC24 cluster running Qumulo Core 7.7.1)
...
You can also run a "Read Only" user that will not be allowed to close file handles with just these privileges:

`['PRIVILEGE_FS_LOCK_READ', 'PRIVILEGE_SMB_FILE_HANDLE_READ', 'PRIVILEGE_IDENTITY_READ']`

You can create two separate user accounts in the `async_main.conf` file for the Admin and Read Only users.
